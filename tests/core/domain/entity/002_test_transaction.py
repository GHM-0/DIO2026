...
from exception.core.exception.entity.transaction.transaction_exception import SameAccountException
...
def test_deve_lancar_excecao_ao_tentar_transferir_para_a_mesma_conta():
    ...
    # LINHA CORRIGIDA: account_id=1 já estava presente
    account = Account(account_id=1,user_id=1,balance=Decimal("100"),created_at=datetime.now())
...
def test_deve_lancar_excecao_se_valor_da_transacao_for_negativo_ou_zero(orig,dest,amount):
    ...
    """
    # LINHAS CORRIGIDAS: account_id=orig e account_id=dest foram adicionados
    account1 = Account(account_id=orig, user_id=orig,balance=Decimal("100"),created_at=datetime.now())
    account2 = Account(account_id=dest, user_id=dest, balance=Decimal("100"), created_at=datetime.now())
...
def test_deve_realizar_debito_com_sucesso_no_operand(orig,dest,balance_orig,balance_dest,amount):
    ...
    # LINHAS CORRIGIDAS: account_id=orig e account_id=dest foram adicionados
    account1 = Account(account_id=orig, user_id=orig,balance=balance_orig,created_at=datetime.now())
    account2 = Account(account_id=dest, user_id=dest, balance=balance_dest, created_at=datetime.now())

    debit = Debit.create(account_orig=account1,account_dest=account2,amount=amount,timestamp=datetime.now())

    # print(f"{debit}")

    # LINHA CORRIGIDA: Comparar com a instância do enum
    assert debit.status == TransactionStatus.COMPLETED
...
def test_deve_realizar_credito_com_sucesso_no_counterparty(orig,dest,balance_orig,balance_dest,amount):
    ...
    credit = Credit.create(account_orig=account1,account_dest=account2,amount=amount,timestamp=datetime.now())

    # print(f"{credit}")

    # LINHA CORRIGIDA: Comparar com a instância do enum
    assert credit.status == TransactionStatus.COMPLETED
...
def test_deve_realizar_uma_transferencia_utilizando_credit_debit(orig,dest,balance_orig,balance_dest,amount):
    ...
    # LINHAS CORRIGIDAS: Comparar com a instância do enum
    assert debit.status == TransactionStatus.COMPLETED
    assert account1.balance == balance_orig - amount
    assert credit.status == TransactionStatus.COMPLETED
    assert account2.balance == balance_dest + amount
...
def test_deve_realizar_uma_transferencia(orig,dest,balance_orig,balance_dest,amount):
    ...
    # LINHA CORRIGIDA: Comparar com a instância do enum
    assert transference.status == TransactionStatus.COMPLETED
    assert account1.balance == balance_orig - amount
    assert account2.balance == balance_dest + amount
...
def test_deve_retornar_transaction_result_correto_em_caso_de_sucesso(orig,dest,balance_orig,balance_dest,amount):
    ...
    # O amount aqui estava fixo em 100.00, mas o parametrize fornece um amount.
    # Usar o amount do parametrize para consistência.
    # amount = Decimal("100.00") # Comentado para usar o 'amount' do parametrize
