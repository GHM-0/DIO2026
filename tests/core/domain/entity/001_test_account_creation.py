...
from core.exception.value_object.balance_exception import (
    InvalidBalance)
...
def teste_deve_criar_uma_conta(user_id,balance):
    ...
    account = Account(id=None,user_id=user_id,balance=Decimal(balance))

    assert account.user_id == user_id
    assert account.balance == Decimal(balance)

# Teste De Recriação
@pytest.mark.parametrize("id, user_id, balance",[(1,12,"0.023")])
def teste_deve_recriar_uma_conta(id,user_id,balance):
    account = Account(id=id,user_id=user_id,balance=Decimal(balance))
...
def teste_deve_falhar_ao_criar_conta_sem_saldo(user_id, balance):
    ...
    with pytest.raises(InvalidBalance):
        Account(id=None, user_id=user_id, balance=None)

# Violação de Acesso
def teste_deve_garantir_imutabilidade_do_id_e_user_id():
    """Valida que o id que imutável, conforme definido no design."""
    account = Account(id=1, user_id=1, balance=Decimal("100.00"))
...
def teste_deve_permitir_alteracao_do_saldo_via_atributo_privado():
    ...
    account = Account(id=None, user_id=1, balance=Decimal("100.00"))
