...
class AccountRepository(IAccountRepository):
    ...
    _IMMUTABLE_FIELDS = {"id", "user_id", "created_at"} # Define immutable fields for AccountModel
    ...
    async def exists_by_id(self, id: int) -> bool:
        ...
        query: Select = select(AccountModel.id).where(AccountModel.id == id)
    ...
    async def read_one(self, id: int) -> Optional[Account]:
        """Recupera uma única entidade pelo ID."""

        query: Select[AccountModel] = select(AccountModel).where(AccountModel.id == id)
        result: Result = await self._session.execute(query)
    ...
    async def update_one(self, entity: Account) -> Optional[Account]:
        ...
        try:
            ...
            query: Select[AccountModel] = select(AccountModel).where(AccountModel.id == entity.id)
            result: Result = await self._session.execute(query)
            model: Optional[AccountModel] = result.scalar_one_or_none()

            if not model:
                raise PersistenceException(f"{Account.__name__} não encontrada pelo id: {entity.id}")
            ...
            raise PersistenceException(
                f"Erro ao atualizar {Account.__name__} id:{entity.id}. Detalhes: {str(e)}"
            )

    async def remove_one(self, id: int) -> bool:
        """Exclui permanentemente um registro."""
        deleted: Delete = delete(AccountModel).where(AccountModel.id == id)
        ...
        try:
           ...
           if result.rowcount > 1:
               raise PersistenceException(
                   f"Erro de integridade: a tentativa de remover {Account.__name__}  id:{id} "
           ...
           return (result.rowcount == 1)
            ...
            raise PersistenceException(f"Falha ao remover {Account.__name__}  id:{id}. Erro original: {str(e)}")
